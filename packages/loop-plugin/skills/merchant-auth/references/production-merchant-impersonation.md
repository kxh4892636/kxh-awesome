...
线上附身必须通过用户已有 Chrome 登录态执行。使用 `@chrome` 插件时，按 Chrome 插件 skill 初始化并获取 `browser = await agent.browsers.get('extension')`，不要改用独立 Playwright 新浏览器；独立浏览器不会继承用户的 ECOP/抖店登录态。

读取 ECOP token 时必须使用 Chrome 插件的 CDP 能力在页面主执行上下文执行 `Runtime.evaluate`。普通 `tab.playwright.evaluate(...)` 可能运行在隔离上下文中，看不到 `window.__PRELOAD_CONTEXT__`，会误判为 `hasPreload=false`、`hasToken=false`。遇到这种情况不要提示用户重新登录，先切到 CDP 主上下文读取。

读取 token 只报告 `hasToken`、`tokenLength` 等非敏感信号，不要输出 token 原文。

### 1. 打开 ECOP 并读取 token

使用 Codex 自带 Chrome 插件打开或切到 ECOP 页面：
...
继续在 ECOP 页面主上下文中调用。推荐把 token 的读取和接口请求放在同一个 `Runtime.evaluate` 表达式中，避免 token 被带出页面上下文：
...
沿用第 2 步的 CDP 主上下文方式调用 `POST /auth/batch_apply?__token=<token>`，请求体为：
...
轮询也在 CDP 主上下文中执行，避免 token 离开页面。每次轮询只返回审批状态、耗时和接口 code/msg，不返回 token。
...
如果 `code === 0` 且 `data` 是 URL，使用 Codex 自带 Chrome 插件打开该 URL。打开后等待页面加载完成，再导航到目标业务页面执行 E2E。
...
| Playwright evaluate 看不到 `__PRELOAD_CONTEXT__` | 不要判定未登录；改用 Chrome 插件 CDP `Runtime.evaluate` 读取页面主上下文 |
