...
| --- | --- | --- | --- |
| `agent-browser` | `https://github.com/vercel-labs/agent-browser/blob/main/skills/agent-browser/SKILL.md` | `packages/code-kit/skills/agent-browser` | 读取上游 `SKILL.md` raw 内容，比较与本地 `packages/code-kit/skills/agent-browser/SKILL.md` 的差异；只有有意义差异时同步。该 skill 是 discovery stub，保留其通过 `agent-browser skills get core` 加载版本匹配工作流的设计，不把 CLI 动态内容内联进本地文件。保留本地 overlay：默认运行 `agent-browser` 命令时使用 `--auto-connect` 连接已有浏览器会话。 |
