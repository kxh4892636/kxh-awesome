...
function findRepoRoot() {
	let dir = process.cwd();
	while (dir !== "/") {
		if (existsSync(join(dir, "connectrpc.config.json"))) return dir;
...
async function ensureBuf() {
	...
	if (existsSync(bufBin)) return bufBin;
	...
	if (process.platform !== "win32") chmodSync(bufBin, 493);
	return bufBin;
}
async function main() {
