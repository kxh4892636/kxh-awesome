...
import type { DailyBar, Security } from "../libs/api/gen/etf-service/etf/v1/etf_pb";
import {
  EtfService,
  GetDailyBarsResponseSchema,
  ListSecuritiesResponseSchema,
} from "../libs/api/gen/etf-service/etf/v1/etf_pb";
import { normalizeBars, type ChartBar } from "../features/market-dashboard/chart-data";

// @connectrpc/connect 是测试专用 devDependency，Transport 结构类型从 TransportProvider props 派生。
type Transport = ComponentProps<typeof TransportProvider>["transport"];
...
/**
 * 在 Connect transport 接缝上 mock EtfService，供 TransportProvider 注入。
 */
export const createEtfTransport = (handlers: MockEtfHandlers): Transport =>
  createRouterTransport(({ service }) => {
    service(EtfService, {
      listSecurities: () => {
        if (handlers.listError) throw handlers.listError;
        return create(ListSecuritiesResponseSchema, {
          securities: handlers.securities ?? [],
        });
      },
      getDailyBars: () => {
        if (handlers.barsError) throw handlers.barsError;
        const security = handlers.security ?? makeSecurity();
        const bars = handlers.bars ?? [];
        return create(GetDailyBarsResponseSchema, {
          security,
          bars,
          meta: {
            cacheStatus: "hit",
...
/** 与 app providers 一致的最小组合（不含 router）：TransportProvider + 每测一新的 QueryClient。 */
