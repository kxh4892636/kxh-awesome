...
const useWheelZoom = (params: {
  ...
  useEffect((): (() => void) | undefined => {
    ...
    // Native non-passive listening lets chart zoom own the wheel only while the pointer is on the plot.
  ...
  return useCallback(
    (event: ReactMouseEvent<HTMLCanvasElement>): void => {
      ...
      dragRef.current = {
        ...
        visibleCount,
        totalCount,
        // Index movement follows candle width so panning feels stable at every canvas size.
