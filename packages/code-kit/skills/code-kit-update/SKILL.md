---
name: code-kit-update
description: 维护 code-kit 内远程来源 skill、reference 模块和嵌入式 guidance 的登记与更新流程。触发场景：更新或检查 code-kit skill、同步 reference 模块、查看远程来源、添加/删除登记项。关键词：code-kit update、remote skill、Git source、reference module、skill registry。
---

# code-kit-update

使用本 skill 让 code-kit 内有远程来源的 skills、reference 模块和嵌入式 guidance 保持可复现。它记录每个来源的名称、远程来源、本地路径、适用时的 reference 模块或嵌入式 guidance 位置，以及预期更新方式。
...
只有当用户明确要求把某个新 skill 或 reference 模块加入 code-kit 远程来源登记表时，才新增登记项。新增时记录：
...
# 检查 <tmp-dir>，将其与 packages/code-kit/skills/<skill-name> 比较，然后有意识地同步。
```
...
2. 检查 `packages/code-kit/skills/code-spec/SKILL.md` 中的嵌入式 guidance 章节。
